...
function pgOspedali(){
 ...
 '<section class="blk"><article class="card"><div class="card__body"><h2>'+esc(t('hospital_price_title'))+'</h2><ul><li>'+esc(t('hospital_price_night'))+'</li><li>'+esc(t('hospital_price_week'))+'</li><li>'+esc(t('hospital_price_long'))+'</li></ul></div></article></section>'+
 '<section class="blk"><h2 class="sec">'+esc(t('hospital_routes'))+'</h2><p class="sub">'+esc(t('hospital_traffic'))+'</p><div class="grid">'+hospitals.map(([name,km,time])=>'<article class="card"><div class="card__body"><h3>'+esc(name)+'</h3><p>≈ '+(LANG==='en'?km.replace(',','.'):km)+' km · '+time+' min '+esc(t('hospital_drive'))+'</p>'+externalLink(directionsLink(name+' Cagliari')+'&travelmode=driving',t('route_from_home'),'btn btn--map')+'</div></article>').join('')+'</div></section>'+
