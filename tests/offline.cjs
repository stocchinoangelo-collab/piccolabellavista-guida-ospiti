...
let failFile='',online=true,invalidAsset=false,redirectedAsset=false;
...
function worker(code){const handlers={};let skipped=false,claimed=false;const self={registration:{scope},location:new URL(scope+'sw.js'),addEventListener:(k,f)=>handlers[k]=f,skipWaiting:async()=>{skipped=true},clients:{claim:async()=>{claimed=true},matchAll:async()=>[]}};vm.runInNewContext(code,{self,caches,URL,Response,fetch:async (u,options)=>{
 if(!online||failFile&&u.endsWith(failFile))throw Error('Network failure');
 if(options){assert.equal(options.cache,'reload');assert.equal(options.redirect,'error');assert.equal(options.credentials,'same-origin');}
 if(invalidAsset&&u.endsWith('.js'))return new Response('<html>Login</html>',{headers:{'Content-Type':'text/html'}});
 const response=require('./asset-response.cjs')(root,scope,u);
 if(redirectedAsset)Object.defineProperty(response,'redirected',{value:true});
 return response;
}});return {handlers,get skipped(){return skipped},get claimed(){return claimed}}}
...
(async()=>{
 ...
 online=true;const updatedSource=source.replace(/consolidated-\d+/,'test-next');
 ...
 failFile='';invalidAsset=true;const login=worker(updatedSource);await assert.rejects(event(login,'install'));assert(!login.skipped);assert(buckets.has(current));assert.equal(buckets.size,3,'Incomplete new cache removed');
 invalidAsset=false;redirectedAsset=true;const redirected=worker(updatedSource);await assert.rejects(event(redirected,'install'));assert(!redirected.skipped);assert(buckets.has(current));
 redirectedAsset=false;const updated=worker(updatedSource);await event(updated,'install');await event(updated,'activate');
