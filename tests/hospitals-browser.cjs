...
for(const [label,width,height] of [['mobile',390,844],['tablet',768,1024],['desktop',1440,1000]]){
 ...
 await p.goto('http://localhost:4173/#ospedali');
 ...
 await c.setOffline(false);await p.goto('http://localhost:4173/#ospedali');assert.equal(await p.locator('main h1').count(),1);
