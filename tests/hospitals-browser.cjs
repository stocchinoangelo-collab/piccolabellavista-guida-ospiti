...
for(const [label,width,height] of [['mobile',390,844],['tablet',768,1024],['desktop',1440,1000]]){
 const c=await b.newContext({viewport:{width,height}});const p=await c.newPage();let errors=[];p.on('pageerror',e=>errors.push(e.message));p.on('console',m=>{if(m.type()==='error')errors.push(m.text())});
 await p.goto('http://127.0.0.1:4173/#ospedali');
 ...
 await p.locator('[data-lang="it"]').click();await p.locator('#burger').click();await p.locator('#menu a[href="#ospedali"]').focus();await p.keyboard.press('Enter');
 ...
 await c.setOffline(false);await p.goto('http://127.0.0.1:4173/#ospedali');assert.equal(await p.locator('main h1').count(),1);
