...
const prefix=arg('--base','/').replace(/\/$/,'');
const mime={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.json':'application/json','.webmanifest':'application/manifest+json','.svg':'image/svg+xml','.jpg':'image/jpeg','.webp':'image/webp','.png':'image/png','.woff2':'font/woff2'};
http.createServer((req,res)=>{
 ...
 if(prefix){if(requested!==prefix&&!requested.startsWith(prefix+'/')){res.writeHead(404);return res.end('Not found')}requested=requested.slice(prefix.length)||'/';}
